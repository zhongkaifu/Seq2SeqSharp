...
namespace ManagedCuda
{
    ...
    public class CudaContext : IDisposable
    {
        ...
        //Cuda12
				public ulong GetId(CudaStream stream)
				{
					ulong streamId = 0;
					var res = DriverAPINativeMethods.Streams.cuStreamGetId(stream.Stream, ref streamId);
            ...
            get
            {
				...
				public ulong Id
				{
					get
					{
						if (disposed) throw new ObjectDisposedException(this.ToString());
						ulong ctxId = 0;
						CUResult res = DriverAPINativeMethods.ContextManagement.cuCtxGetId(_context, ref ctxId);
						Debug.WriteLine(String.Format("{0:G}, {1}: {2}", DateTime.Now, "cuCtxGetFlags", res));
						if (res != CUResult.Success)
							throw new CudaException(res);
						return ctxId;
