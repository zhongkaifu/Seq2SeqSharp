...
namespace ManagedCuda
{
	...
	{
		private readonly CUmemoryPool _memoryPool;
		...
		public CudaMemoryPool(CUmemPoolProps props)
		{
			...
			res = DriverAPINativeMethods.MemoryManagement.cuMemPoolCreate(ref _memoryPool,
				ref props);
			Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuMemPoolCreate", res);
		...
		public CudaMemoryPool(nint handle, CUmemAllocationHandleType handleType, ulong flags)
		{
			_memoryPool = new CUmemoryPool();
			res = DriverAPINativeMethods.MemoryManagement.cuMemPoolImportFromShareableHandle(
				ref _memoryPool, handle, handleType, flags);
			Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuMemPoolImportFromShareableHandle",
				res);
			if (res != CUResult.Success)
				throw new CudaException(res);
			IsOwner = true;
		...
		public CudaMemoryPool(CUdevice device, bool isDefault)
		{
			...
			if (isDefault)
			{
				res = DriverAPINativeMethods.DeviceManagement.cuDeviceGetDefaultMemPool(
					ref _memoryPool, device);
				Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuDeviceGetDefaultMemPool", res);
				if (res != CUResult.Success)
					throw new CudaException(res);
				IsOwner = false;
			}
			...
			{
				res = DriverAPINativeMethods.DeviceManagement.cuDeviceGetMemPool(ref _memoryPool,
					device);
				Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuDeviceGetMemPool", res);
				if (res != CUResult.Success)
					throw new CudaException(res);
				IsOwner = true;
			}
		...
		protected virtual void Dispose(bool fDisposing)
		{
			if (fDisposing && !disposed)
			{
				if (IsOwner)
				{
					...
					Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuMemPoolDestroy", res);
				}
				disposed = true;
			}
			if (!fDisposing && !disposed)
				Debug.WriteLine("ManagedCUDA not-disposed warning: {0}", GetType());
		}
		#endregion
		#region Methods
		/// <summary>
		...
		{
			res = DriverAPINativeMethods.MemoryManagement.cuMemPoolTrimTo(_memoryPool,
				minBytesToKeep);
			Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuMemPoolTrimTo", res);
			if (res != CUResult.Success)
				throw new CudaException(res);
		}
		...
		/// <returns></returns>
		public CudaDeviceVariable<T> ImportPointer<T>(CUmemPoolPtrExportData shareData)
			where T : struct
		{
			var devPtr = new CUdeviceptr();
			res = DriverAPINativeMethods.MemoryManagement.cuMemPoolImportPointer(ref devPtr,
				_memoryPool, ref shareData);
			Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuMemPoolImportPointer", res);
			if (res != CUResult.Success)
				throw new CudaException(res);
		...
		public CudaDeviceVariable<T> MemAllocFromPoolAsync<T>(SizeT bytesize, CudaStream hStream)
			where T : struct
		{
			var devPtr = new CUdeviceptr();
			res = DriverAPINativeMethods.MemoryManagement.cuMemAllocFromPoolAsync(ref devPtr,
				bytesize, _memoryPool, hStream.Stream);
        ...
        public CUmemAccess_flags GetAccess(CUmemLocation location)
		{
			...
			res = DriverAPINativeMethods.MemoryManagement.cuMemPoolGetAccess(ref flags, _memoryPool,
				ref location);
			Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuMemPoolGetAccess", res);
			if (res != CUResult.Success)
				throw new CudaException(res);
			return flags;
		}
		...
		{
			res = DriverAPINativeMethods.MemoryManagement.cuMemPoolSetAccess(_memoryPool,
				accessDescs, accessDescs.Length);
			Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuMemPoolSetAccess", res);
			if (res != CUResult.Success)
				throw new CudaException(res);
		}
		...
		/// <param name="flags">must be 0</param>
		public nint ExportToShareableHandle(CUmemAllocationHandleType handleType, ulong flags)
		{
			var ret = new nint();
			res = DriverAPINativeMethods.MemoryManagement.cuMemPoolExportToShareableHandle(ref ret,
				_memoryPool, handleType, flags);
			Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuMemPoolExportToShareableHandle",
				res);
			if (res != CUResult.Success)
				throw new CudaException(res);
			return ret;
		...
		{
			res = DriverAPINativeMethods.MemoryManagement.cuMemPoolSetAttribute(_memoryPool, attr,
				ref value);
			Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuMemPoolSetAttribute", res);
			if (res != CUResult.Success)
				throw new CudaException(res);
		}
		...
		{
			res = DriverAPINativeMethods.MemoryManagement.cuMemPoolGetAttribute(_memoryPool, attr,
				ref value);
			Debug.WriteLine("{0:G}, {1}: {2}", DateTime.Now, "cuMemPoolGetAttribute", res);
			if (res != CUResult.Success)
				throw new CudaException(res);
		}
		...
		/// </summary>
		public CUmemoryPool MemoryPool => _memoryPool;
		/// <summary>
