...
kernel void kernel_soft_max_back_f32(
    ...
    device const float * pdy = (device const float *) (src0 + i01*args.nb01 + i02*args.nb02 + i03*args.nb03);
    device const float * py  = (device const float *) (src1 + i01*args.nb11 + i02*args.nb12 + i03*args.nb13);
    device       float * pdx = (device       float *) (dst  + i01*args.nb1  + i02*args.nb2  + i03*args.nb3);
    ...
    for (int i00 = tpitg.x; i00 < args.ne00; i00 += tptg.x) {
        pdx[i00] = py[i00] * (pdy[i00] - sum) * args.scale;
    }
}
...
kernel void kernel_cross_entropy_loss_f32(
    ...
    device float * pout = (device float *) dst;
    float total_loss = 0.0f;

    for (int64_t i03 = 0; i03 < args.ne03; ++i03) {
        for (int64_t i02 = 0; i02 < args.ne02; ++i02) {
            for (int64_t i01 = 0; i01 < args.ne01; ++i01) {
                device const float * plogits = (device const float *) (src0 + i01*args.nb01 + i02*args.nb02 + i03*args.nb03);
                device const float * plabels = (device const float *) (src1 + i01*args.nb11 + i02*args.nb12 + i03*args.nb13);

                float lmax = -INFINITY;
                for (int64_t i00 = tpitg.x; i00 < args.ne00; i00 += tptg.x) {
                    lmax = MAX(lmax, plogits[i00]);
                }
                ...
                float ldot = 0.0f;
                for (int64_t i00 = tpitg.x; i00 < args.ne00; i00 += tptg.x) {
                    const float logit = plogits[i00];
                    lsum += exp(logit - max_val);
                    ldot += plabels[i00] * logit;
                }
                ...
                if (tpitg.x == 0) {
                    total_loss += (max_val + log(sum)) - dot;
                }
    ...
    if (tpitg.x == 0) {
        pout[0] = total_loss / ((float) args.ne01 * (float) args.ne02 * (float) args.ne03);
    }
}
...
kernel void kernel_cross_entropy_loss_back_f32(
    ...
    device const float * pgrad   = (device const float *) src0;
    device const float * plogits = (device const float *) (src1 + i01*args.nb11 + i02*args.nb12 + i03*args.nb13);
    device const float * plabels = (device const float *) (src2 + i01*args.nb21 + i02*args.nb22 + i03*args.nb23);
    device       float * pdst    = (device       float *) (dst  + i01*args.nb1  + i02*args.nb2  + i03*args.nb3);
    ...
    float lsum = 0.0f;
    for (int64_t i00 = tpitg.x; i00 < args.ne00; i00 += tptg.x) {
        const float exp_value = exp(plogits[i00] - max_val);
        lsum += exp_value;
        pdst[i00] = exp_value;
    }
    ...
    const float scale = pgrad[0] * args.inv_nr;
    const float inv_sum = 1.0f/sum;

    for (int64_t i00 = tpitg.x; i00 < args.ne00; i00 += tptg.x) {
        pdst[i00] = (pdst[i00] * inv_sum - plabels[i00]) * scale;
    }
}
