    ...
    switch (tensor->op) {
        ...
        case GGML_OP_NORM: {
            if (src0_needs_grads) {
                ...
                struct ggml_tensor * src0_rows   = ggml_is_contiguous_rows(src0)   ? src0   : ggml_cont(ctx, src0);
                struct ggml_tensor * grad_rows   = ggml_is_contiguous_rows(grad)   ? grad   : ggml_cont(ctx, grad);
                struct ggml_tensor * tensor_rows = ggml_is_contiguous_rows(tensor) ? tensor : ggml_cont(ctx, tensor);

                const float inv_ne0 = 1.0f / src0->ne[0];

                struct ggml_tensor * mean = ggml_scale_impl(ctx, ggml_sum_rows(ctx, src0_rows), inv_ne0, 0.0f, false);
                struct ggml_tensor * centered = ggml_sub(ctx, src0_rows, ggml_repeat(ctx, mean, src0_rows));
                struct ggml_tensor * variance = ggml_scale_impl(ctx, ggml_sum_rows(ctx, ggml_mul(ctx, centered, centered)), inv_ne0, 0.0f, false);
                struct ggml_tensor * sigma = ggml_sqrt(ctx, ggml_scale_bias(ctx, variance, 1.0f, eps));
                struct ggml_tensor * grad_mean = ggml_scale_impl(ctx, ggml_sum_rows(ctx, grad_rows), inv_ne0, 0.0f, false);
                struct ggml_tensor * grad_x_hat_mean = ggml_scale_impl(ctx, ggml_sum_rows(ctx, ggml_mul(ctx, grad_rows, tensor_rows)), inv_ne0, 0.0f, false);
                struct ggml_tensor * centered_grad = ggml_sub(ctx, grad_rows, ggml_repeat(ctx, grad_mean, src0_rows));
                struct ggml_tensor * normalized_term = ggml_mul(ctx, tensor_rows, ggml_repeat(ctx, grad_x_hat_mean, src0_rows));
                struct ggml_tensor * dx_inner = ggml_sub(ctx, centered_grad, normalized_term);
                struct ggml_tensor * dx = ggml_div(ctx, dx_inner, ggml_repeat(ctx, sigma, src0_rows));
