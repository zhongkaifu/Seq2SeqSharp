...
API sentencepiece::SentencePieceProcessor* __SP_Init(wchar_t* modelFilename, wchar_t* vocabFilename, int threshold)
{    
    sentencepiece::SentencePieceProcessor* sp = new sentencepiece::SentencePieceProcessor();

    absl::string_view model_filename((char*)modelFilename, lstrlenW(modelFilename));
    CHECK_OK(sp->Load(model_filename));

    if (vocabFilename) {
        absl::string_view vocab_filename((char*)vocabFilename, lstrlenW(vocabFilename));
        CHECK_OK(sp->LoadVocabulary(vocab_filename, threshold));
    }
    return (sp);
}

API void __SP_Finalize(sentencepiece::SentencePieceProcessor* sp)
{
    delete sp;
}

API char* __SP_Encode(sentencepiece::SentencePieceProcessor* sp, char* input, int len)
{
...
API char* __SP_Decode(sentencepiece::SentencePieceProcessor* sp, char* input, int len)
{    
    absl::string_view line(input, len);
...
API void __SP_Free(char* result)
{
    delete result;
}